classdef Circulation
    ...
    methods
        ...
        function [state_derivatives] = get_derivative(obj, t, x)
            ...
            if x(2) > x(1)
                state_derivatives = obj.filling_phase_dynamic_matrix(t) * x;
            elseif x(4) > 0 || x(1) > x(3)
                state_derivatives = obj.ejection_phase_dynamic_matrix(t) * x;
            else
                state_derivatives = obj.isovolumic_phase_dynamic_matrix(t) * x;
            end
        end
        ...
        function [A] = filling_phase_dynamic_matrix(obj, t)
            ...
            A = [(del_dt/el - el/obj.R2), (el/obj.R2), 0, 0;
                1/(obj.R2*obj.C2), -(obj.R1 + obj.R2)/(obj.R1*obj.C2*obj.R2), 1/(obj.R1*obj.C2), 0;
                0, 1/(obj.R1*obj.C3), -1/(obj.R1*obj.C3), 0;
                0, 0, 0, 0];
        end
        ...
        function [time, y] = simulate(obj, total_time)
            ...
            fun = @(t, x) obj.get_derivative(t, x);
            [time, y] = ode45(fun, tspan, initialCondition, options);
        ...
        %%% Verification - Numerical Error
        function [time, y] = simulate_verification_numerical_error(obj, total_time, rel_tol, abs_tol, max_step)
            % Inputs
            % total_time: seconds to simulate
        ...
        function [time, y] = simulate_validation_initial_condition(obj, total_time, initialCondition)
            % Inputs
            % total_time: seconds to simulate
